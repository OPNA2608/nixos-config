...
stdenv.mkDerivation rec {
  ...
  # fetchzip requires a custom unpackPhase to handle dmg
  # fetchurl cannot handle undmg producing >1 directory without this
  sourceRoot = ".";
  ...
  installPhase =
    let
      gameArch = "${if stdenv.hostPlatform.isDarwin then "darwin" else "linux"}-${
        if stdenv.hostPlatform.isx86_64 then "x86_64" else "i686"
      }";
      gameSource =
        if stdenv.hostPlatform.isDarwin then "'Katawa Shoujo'.app" else "'Katawa Shoujo'-${version}-linux";
      gameInstallDir =
  ...
  meta = with lib; {
    ...
    longDescription = ''
      ...
      when a congenital heart defect forces him to move to a new school after a long hospitalization.
      Despite his difficulties, Hisao is able to find friends—and perhaps love, if he plays his cards right.
      There are five main paths corresponding to the 5 main female characters, each path following the storyline
      pertaining to that character.
      ...
      international team of amateur developers, and is available free of charge under the
